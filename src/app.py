...
# Atendimento
    ...
    def telaAtendimento(self):
        ...
        self.buttonCalendarAtendimento.place(relx=0.464, rely=0.213, relwidth=0.03, relheight=0.13)
        ...
        self.buttonCalendarAtendimentoFinal.place(relx=0.664, rely=0.213, relwidth=0.03, relheight=0.13)

        self.frameTvAtendimentos()
        
        self.treeviewAtendimento = ttk.Treeview(self.frameTvAtd, columns=(
            'Data', 'Hora', 'Protocolo', 'Cod.Atendimento', 'Cod.Cliente', 'Nome do Cliente','Procedimento' ,'Funcionario', 'Especialidade', 'Valor', 'Status'), show='headings')
    ...
    def atdAtendido(self):
        ...
        #     messagebox.showinfo("Zenix", "Atendimento já realizado", parent=self.atendimento)
            ...
            self.ModalAtAtendido.grab_set()
            self.ModalAtAtendido.lift()
            self.ModalAtAtendido.title('Atendimento - [Atendido]')
            self.ModalAtAtendido.geometry('650x450')
            self.ModalAtAtendido.configure(background='#D3D3D3')
        ...
        # Aba Funcionario
            ...
            self.treeviewModalAtdAtendido.column("Data", stretch=False, width=95)
            self.treeviewModalAtdAtendido.column("Hora", stretch=False, width=90)
            self.treeviewModalAtdAtendido.column("Protocolo", stretch=False, width=90)
            ...
            verticalBar = ttk.Scrollbar(self.ModalAtAtendido, orient='vertical', command=self.treeviewModalAtdAtendido.yview)
            horizontalBar = ttk.Scrollbar(self.ModalAtAtendido, orient='horizontal', command=self.treeviewModalAtdAtendido.xview)
    ...
    def triggerLancamentoAtd(self, atendimento: list):
        ...
        protocolo = int(self.protocoloAgendaAtendimento)
        data = self.dataAtendimento2
        atendimentos = self.dao.atendimentosAtendidos(protocolo, data)
        for row in atendimentos:
            FuncAtendimento = self.dao.funcionarioNome(row[6])
            percentil = [item[13] for item in FuncAtendimento]
            percentilList.append(percentil[0])
            vlBruto.append(row[8])
        
        vlFuncionarioList = []
        percentilLojaList = []
        vlLojaList = []
        for vlProcedimento, percFunc in zip(vlBruto, percentilList):
            resultado = vlProcedimento * (percFunc/100)
            vlFunc = "{:.2f}".format(resultado)
            vlFuncionarioList.append(vlFunc)
            percLoja = 100 - percFunc
            vlLoja = vlProcedimento * (percLoja/100)
            vlLojaFormatado = "{:.2f}".format(vlLoja)
